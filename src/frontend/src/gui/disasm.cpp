...
void Disasm::Disassembly(Gui* gui, core_t* core) {
  u32 instructions[25] = {};
  ...
  ImGui::Begin("Disassembly", &gui->show_disasm, 0);
  ...
  if(count > 0) {
    for(size_t j = 0; j < count; j++) {
      ...
      switch(j) {
      case 12 ... 14:
        ImGui::TextColored(colors_disasm[j & 3], "0x%" PRIx64 ":\t%s", insn[j].address, insn[j].mnemonic);
        ImGui::SameLine(window_size.x - font_size - 10, -1);
        ImGui::TextColored(colors_disasm[j & 3], "%s", insn[j].op_str);
        break;
      default:
    ...
    ImGui::Text(gui->rom_loaded ? "Could not disassemble instruction!" : "No game loaded!");
  }
  ImGui::EndChild();
  ImGui::End();
}
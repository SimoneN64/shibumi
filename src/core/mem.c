...
u8 read8(mem_t* mem, u32 vaddr) {
  ...
  switch(paddr) {
    ...
    default: logfatal("[ERR] Unimplemented %s[%08X] 8-bit read\n", regions_str(paddr), paddr);
  }
}
...
u16 read16(mem_t* mem, u32 vaddr) {
  ...
  switch(paddr) {
    ...
    default: logfatal("[ERR] Unimplemented %s[%08X] 16-bit read\n", regions_str(paddr), paddr);
  }
}
...
u64 read64(mem_t* mem, u32 vaddr) {
  ...
  switch(paddr) {
    case 0x00000000 ... 0x007FFFFF: result = raccess(64, mem->rdram, paddr); break;
    case 0x04001000 ... 0x04001FFF: result = raccess(64, mem->imem, paddr & IMEM_DSIZE); break;
    default: logfatal("[ERR] Unimplemented %s[%08X] 64-bit read\n", regions_str(paddr), paddr);
  }

  logdebug("64-bit read  %016lX from %s[%08X]\n", result, regions_str(paddr), paddr);
...
void write8(mem_t* mem, u32 vaddr, u8 val) {
  ...
  switch(paddr) {
    ...
    default: logfatal("[ERR] Unimplemented %s[%08X] 8-bit write (%02X)\n", regions_str(paddr), paddr, val);
  }
}
...
void write16(mem_t* mem, u32 vaddr, u16 val) {
  ...
  switch(paddr) {
    ...
    default: logfatal("[ERR] Unimplemented %s[%08X] 16-bit write (%04X)\n", regions_str(paddr), paddr, val);
  }
}
...
void write64(mem_t* mem, u32 vaddr, u64 val) {
  ...
  switch(paddr) {
    ...
    default: logfatal("[ERR] Unimplemented %s[%08X] 64-bit write (%16lX)\n", regions_str(paddr), paddr, val);
  }

  logdebug("64-bit write %016lX to   %s[%08X]\n", val, regions_str(paddr), paddr);
}